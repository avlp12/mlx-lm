...
class DeepseekV32Model(nn.Module):
    ...
    def __call__(
        ...
        _trace = _os.environ.get("ALIS_TRACE")
        if _trace:
            mx.eval(h)
            print(f"[trace r{pipeline_rank}] pre-loop ok (embed+mask+recv), num_layers={self.num_layers} start_idx={self.start_idx} eval_every={_eval_every}", flush=True)
        for i in range(self.num_layers):
            ...
            if _trace:
                mx.eval(h)
                print(f"[trace r{pipeline_rank}] layer {self.start_idx + i} done", flush=True)
        ...
        if pipeline_size > 1:
            n = h.shape[0]  # capture the local batch BEFORE all_gather grows axis 0
            h = mx.distributed.all_gather(h, stream=mx.cpu)
            # Complete the blocking collective on the CPU stream BEFORE any GPU op
            # (the slice below / final norm) consumes it. Otherwise the GPU command
            # buffer sits waiting on the collective while the slowest pipeline rank
            # finishes its whole forward (esp. first-forward kernel compilation),
            # tripping the macOS ~5s GPU watchdog on the rank that arrived first.
            mx.eval(h)
            h = h[:n]  # slice back to the local batch (all_gather stacked all ranks)
